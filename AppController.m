...
- (void)startFireDateTimer:(double)_delay
{
    DDLogVerbose(@"Entering 'startFireDateTimer'.");

    double delta = (0.5 + _delay) * -1.00;
...
- (void)playNext
{
    DDLogVerbose(@"Entering 'playNext'. Number %lu", nextIndex);
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    NSString *dataPath = [NSString stringWithFormat:@"%@/config.plist", library];
    ...
    NSNumber * delayValue = [dataDict valueForKey:@"delay"];

    if (delayValue == NULL) {
        delay = 0.0;
    } else {
        delay = [delayValue doubleValue];
    }
...
- (void)onCommand:(NSString *)command
{
    ...
    if ([command hasPrefix:@"START"]) {
        ...
        nextIndex = [[command substringFromIndex:5] intValue];
        if (delay > 0.0) {
            [self performSelector:@selector(playNext) withObject:nil afterDelay:delay ];
        }else {
            [self playNext];
        }
        ...
        if ([command length] > 5 && [[command substringFromIndex:5] hasPrefix:@"NEXT"]) {
            ...
            nextIndex = [[command substringFromIndex:10] intValue];
            [self playNext];
